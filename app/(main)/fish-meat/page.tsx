...
export default function FishMeatPage() {
  ...
  useEffect(() => {
    async function fetchFishMeat() {
      setIsLoading(true);
      setFetchError(null);

      try {
        // Matches both 'fish-meat' and 'fish_meat' to handle all records
        const { data, error } = await supabase
          .from("products")
          .select("id, name, price, unit, category, image_url, is_active, is_best_seller")
          .in("category", ["fish-meat", "fish_meat"])
          .eq("is_active", true)
        ...
        // Pre-populate card drafts with current cart values or defaults
        ...
        console.error("Failed to fetch fish & meat:", err);
        setFetchError("Could not load fresh cuts. Please refresh.");
      } finally {
        setIsLoading(false);
      }
    }
    ...
    <div className="min-h-screen bg-background text-text flex flex-col font-sans">
      <header className="w-full pt-12 pb-8 px-6 flex flex-col items-center text-center">
        ...
        <p className="text-text/60 max-w-lg leading-relaxed text-sm md:text-base">
          Daily catch and fresh halal cuts sourced and delivered in Dhanmondi.
        </p>
      </header>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {!isLoading && !fetchError && products.length === 0 && (
              <div className="col-span-full py-24 flex flex-col items-center justify-center text-center bg-[#F0EDE6] rounded-2xl border border-[#E3DFD5]">
                ...
                <h3 className="text-xl font-serif font-bold text-gray-900 mb-2">0 items available</h3>
                <p className="text-gray-600 font-medium">Fresh supply arriving tomorrow at 5 AM!</p>
              </div>
            )}
          </div>
        </section>

        {/* ── Sticky Cart Sidebar (md:top-24 prevents navbar overlap) ──────────────── */}
        <aside
          className={`
